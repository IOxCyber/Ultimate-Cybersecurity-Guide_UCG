## User Interface
- Dashboard: (AntiMalware by default)
- Detection: (Incidents)
- Hunting: (Events, Exemption, Advanced Hunting)
- Assets
- Response: (Remediation,Activity, Rule Management, Action)
- Forensics
- Notification
- Configuration (profile, asset Configuration, Block list, Exception Rules)



## 1. EDR (Endpoint Detection & Response):
- works on Behaviour based monitoring
- Monitors Process, File, Registery, PATH, System Files.
- Can kill malicious processes & Quarantine a file, asset (but only connected with Qualys for analysis)

## 2. EDR (Behaviour Based Monitoring) & Anti Malware Engine (Add on feature/ Signature Based Monitoring)

## 3. EDR Module working:
1. Discover & Monitor::
- Collect Data from Asset via Qagent to QConsole.

2. Detect & Investigate::
- Stored & Analysis of received data.
- Correlate data into Events/INC, Threat Hunting.

3. Response & Prevent::
- Quarantine, Patch or any configuration changes.



